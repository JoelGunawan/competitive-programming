...
struct segment_tree {
  ...
  void update(vector<pair<int, int>> &p) {
    head.pb(v.size());
    v.pb(node());
    update(head.back(), head[(int)head.size() - 2], 0, lim - 1, p);
  }
  void update(int c, int pr, int cl, int cr, vector<pair<int, int>> &p) {
    v[c] = v[pr];
    if(cl == cr) {
      // update
      while(p.size() && p.back().fi == cl)
        v[c].val = mul(v[c].val, p.back().se), p.pop_back();
      return;
    }
    int mid = (cl + cr) >> 1;
    ...
    v[c].val = mul(v[v[c].l].val, v[v[c].r].val);
  }
...
int main() {
  ...
  vector<int> primes;
  const int lim2 = 4e5 + 5;
  bool prime[lim2];
  memset(prime, 1, sizeof(prime));
  for(int i = 2; i < lim2; ++i) {
    if(prime[i]) {
      primes.pb(i);
      for(ll j = 2 * i; j < lim2; j += i)
        prime[j] = 0;
    }
  }
  // fi -> idx, se -> power
