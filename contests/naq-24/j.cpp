...
bool comp(pair<bigint, bigint> a, pair<bigint, bigint> b) {
  return a.fi * b.se > a.se * b.fi;
}
bool comp2(pair<bigint, bigint> a, pair<bigint, bigint> b) {
  // cerr << "COMP " << (ll)a.fi << " " << (ll)a.se << " " << (ll)b.fi << " " << (ll)b.se << endl;
  return a.fi * b.se <= a.se * b.fi;
}
bool check_point(vector<pair<ll, ll>> in) {
  bigint f1[3] = {0, 0, 0}, f2[3] = {3, 3, 3}, f3[3] = {3, 3, 3};
  // in each iteration, check 2 in
  vector<pair<bigint, bigint>> v;
  for(int i = 0; i < 3; ++i)
    v.pb(mp(bigint(in[i].fi), bigint(in[i].se)));
  // if 2 in -> correct
  // if not 2 in -> not correct
  for(int i = 1; i <= l; ++i) {
    int wrong = 0;
    for(int j = 0; j < 3; ++j) {
      if(comp(mp(v[j].fi, v[j].se), mp(f1[j] + 1, f3[j])) && comp(mp(f1[j] + 2, f3[j]), mp(v[j].fi, v[j].se))) {
        ++wrong;
        f2[j] = f1[j] + 2;
        f1[j] += 1;
      }
      else {
        if(comp2(mp(v[j].fi, v[j].se), mp(f1[j] + 1, f3[j]))) {
          f2[j] = f1[j] + 1;
        }
        else {
          f1[j] += 2;
      ...
      f1[j] *= 3, f2[j] *= 3, f3[j] *= 3;
    }
    if(wrong > 1)
      return false;
    // cerr << (ll)f1 << " " << (ll)f2 << endl;
    if(i >= 20000)
      break;
  }
  if(l >= 20000) {
    // cerr << (ll)(d * f1) << " " << (ll)(d * f2) << " " << (ll)(n * f3) << endl;
    int wrong = 0;
    for(int j = 0; j < 3; ++j) {
      if(v[j].se * f1[j] != v[j].fi * f3[j] && v[j].se * f2[j] != v[j].fi * f3[j])
        ++wrong;
    }
    return true;
    // check on point or not
  }
  return true;
}
int main() {
  ...
  if(check_point({mp(xn, xd), mp(yn, yd), mp(zn, zd)}))
    cout << 1 << endl;
  else 
    cout << 0 << endl;
  return 0;
