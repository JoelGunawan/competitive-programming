...
def toll():
    ...
    segmtree = [ [INF] * (k**2) for i in range(2 * offset)]
    mingroup = [ i-offset for i in range(2*offset)]
    maxgroup = [ i+1-offset for i in range(2*offset)]
    
    for i in range(m):
        a, b, t = map(int, sys.stdin.readline().split())
        segmtree[offset + (a // k)][((a%k)*k) + (b%k)] = t
    
    sys.stderr.write("Read input\n")
    ...
    def getgroup(a, b):
        ...
        for node in leftside:
            combine(res, segmtree[node], tmp, k)
            tmp, res = res, tmp
            
        for i in range(len(rightside)-1, -1, -1):
            combine(res, segmtree[rightside[i]], tmp, k)
            tmp, res = res, tmp
    ...
    for order in allorders:
        ...
        if group_a >= group_b:
            ...
            matrix = getgroup(group_a, group_b) # 1 is root
            print(matrix[((a%k)*k)+(b%k)] if matrix[((a%k)*k)+(b%k)] != INF else -1)
